...
int main()
{
	wchar_t temp[100]; // unicode 编码存储字符用的
	int i = 666; // 需要输出的整型变量
	initgraph(640, 480); // 创建一个宽 640 x 高 480 像素的窗口
	setbkmode(OPAQUE);
	...
	// 4.设置文字大小和字体，参数依次为：高、宽、字体。若宽为 0 则表示自适应,
	...
	// 将变量内容格式化写入到中间变量 temp 中
	// 上述注释的语句展示了一些格式控制符的使用示例，自行将上述注释仅打开一条查看
	outtextxy(20, 30, temp);
	// 5.输出带背景的文字,在指定的坐标(20,30)处输出 Hello 666 展示输出字符变量内容
	// _T(“*”)是将*转换为 unicode 编码的一种常见方式,unicode 编码内容参见 1.2 节
